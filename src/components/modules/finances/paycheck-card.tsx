...
export function PaycheckCard({
  ...
  const remaining = paycheck.net_amount - recurringExpensesTotal;
  const pct =
    paycheck.net_amount > 0 ? Math.min(100, (recurringExpensesTotal / paycheck.net_amount) * 100) : 0;
      ...
      <CardContent className="flex flex-col gap-3">
        ...
        <div>
          <div className="mb-1 flex items-center justify-between text-xs text-muted-foreground">
            <span>Recurring expenses {formatCurrency(recurringExpensesTotal)}</span>
            <span>
